static class Templates
{
    ...
    internal static List<(string Path, string Source)> GetFiles(ProtocolDefinition jsDef, ProtocolDefinition browserDef)
    {
        ...
        var res = new List<(string Path, string Source)>
        {
            ("ChromeSession_Domains.cs", GetChromeSession_Domains(domains)),
            ("CommandResponseTypeMap.cs", GetCommandResponseTypeMap(domains)),
            ("EventTypeMap.cs", GetEventTypeMap(domains))
        };
        return res;
    }
...
{{
    ...
    /// </summary>
    public partial class {name}Adapter
    {{
...
{(def.Events == null ? "" : Environment.NewLine + string.Join(Environment.NewLine, def.Events.Select(v => GetSource(v))))}
    ...
    $@"        /// <summary>
        ...
        /// </summary>
        public async Task<{name}CommandResponse> {name}({name}Command command{(def.Parameters == null ? " = null" : "")}, CancellationToken cancellationToken = default, int? millisecondsTimeout = null, bool throwExceptionIfResponseNotReceived = true)
        {{
...
{{
    ...
    {{
{ string.Join(Environment.NewLine, def.Enum.Select(v => @$"        [JsonStringEnumMemberName(""{v}"")]
    ...
    static string GetSource(PropertiyDefinition def, bool crutch = false)
    {
        ...
        /// {def.Description.ToRn().Replace("\n", "\n        /// ")}
        ///{(crutch ? "" : " ")}</summary>") + $@"
        [JsonPropertyName(""{def.Name}"")]{(def.Optional == true ? $@"
