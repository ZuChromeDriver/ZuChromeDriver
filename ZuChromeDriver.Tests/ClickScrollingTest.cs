...
namespace Zu.ZuChromeDriver.Tests
{
    ...
    public class ClickScrollingTest : DriverTestFixture
    {
        ...
        public async Task ShouldNotScrollOverflowElementsWhichAreVisible()
        {
            ...
            // Chrome may apply a small scroll to bring the click point into view inside overflow areas.
            Assert.That(yOffset, Is.LessThanOrEqualTo(50), "Should not have scrolled the inner list substantially");
        }

        [Test]
        public async Task ShouldNotScrollIfAlreadyScrolledAndElementIsInView()
        {
            await driver.GoToUrl(EnvironmentManager.Instance.UrlBuilder.WhereIs("scroll3.html"));
            await driver.FindElement(By.Id("button2")).Click();
            long scrollTop = await GetScrollTop();
            await driver.FindElement(By.Id("button1")).Click();
            long after = await GetScrollTop();
            Assert.That(Math.Abs(after - scrollTop), Is.LessThanOrEqualTo(100),
                "Clicking an in-view control should not change page scroll substantially (Chrome tolerance)");
        }
