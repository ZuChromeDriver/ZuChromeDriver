...
namespace Zu.ZuChromeDriver.Tests
{
    ...
    public class I18Test : DriverTestFixture
    {
        ...
        [Test]
        [Ignore("ChromeDriver supports BMP only for SendKeys; Selenium IgnoreBrowser(Chrome|Edge)")]
        public async Task ShouldBeAbleToEnterSupplementaryCharacters()
        {
            await driver.GoToUrl(chinesePage);
            ...
            IWebElement el = await driver.FindElement(By.Name("i18n"));
            await el.SendKeys(input);

            Assert.AreEqual(input, await el.GetAttribute("value"));
        }
