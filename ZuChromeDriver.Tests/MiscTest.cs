...
namespace Zu.ZuChromeDriver.Tests
{
    ...
    public class MiscTest : DriverTestFixture
    {
        ...
        [Test]
        [Ignore("Chrome returns XML PageSource formatted for display as HTML document (Selenium IgnoreBrowser Chrome).")]
        public async Task ShouldBeAbleToGetTheSourceOfAnXmlDocument()
        {
            await driver.GoToUrl(simpleXmlDocument);
            string source = (await driver.PageSource()).ToLower();
