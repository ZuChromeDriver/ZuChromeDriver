...
namespace Zu.ZuChromeDriver.Tests
{
    ...
    public class AlertsTest : DriverTestFixture
    {
        ...
        [Test]
        public async Task SettingTheValueOfAnAlertThrows()
        {
            await driver.GoToUrl(CreateAlertPage("cheese"));
            ...
            IAlert alert = await WaitFor(AlertToBePresent(), "No alert found");
            try {
                await AssertEx.ThrowsAsync<ElementNotInteractableException>(async () => await alert.SendKeys("cheese"));
            } finally {
                await alert.Accept();
            }
        }
        ...
        [Test]
        public async Task AlertShouldNotAllowAdditionalCommandsIfDismissed()
        {
            await driver.GoToUrl(CreateAlertPage("cheese"));
            ...
            await alert.Dismiss();
            string text;
            await AssertEx.ThrowsAsync<NoAlertPresentException>(async () => text = await alert.Text());
        }
        ...
        public async Task SwitchingToMissingAlertThrows()
        {
            ...
            await AssertEx.ThrowsAsync<NoAlertPresentException>(async () => await AlertToBePresent());
        }
        ...
        [Test]
        [Ignore("Upstream Selenium ignores Chrome/Edge because onload alerts in another window can hang.")]
        public async Task ShouldNotHandleAlertInAnotherWindow()
        {
            string pageWithOnLoad = EnvironmentManager.Instance.UrlBuilder.CreateInlinePage(new InlinePage()
                .WithOnLoad("javascript:alert(\"onload\")")
                .WithBody("<p>Page with onload event handler</p>"));
            ...
            try {
                await driver.FindElement(By.Id("open-new-window")).Click();
                List<string> allWindows = new(await driver.WindowHandles());
                allWindows.Remove(mainWindow);
                Assert.That(allWindows, Has.One.Items);
                onloadWindow = allWindows[0];

                await AssertEx.ThrowsAsync<WebDriverException>(async () => {
                    IWebElement el = await driver.FindElement(By.Id("open-new-window"));
                    await WaitFor(AlertToBePresent, TimeSpan.FromSeconds(5), "No alert found");
                });

            } finally {
                await driver.SwitchTo().Window(onloadWindow);
                IAlert alert = await WaitFor(AlertToBePresent(), "No alert found");
                await alert.Dismiss();
                await driver.Close();
                await driver.SwitchTo().Window(mainWindow);
        ...
        public async Task IncludesAlertTextInUnhandledAlertException()
        {
            ...
            await driver.FindElement(By.Id("alert")).Click();
            await WaitFor(AlertToBePresent(), "No alert found");

            await AssertEx.ThrowsAsync<UnhandledAlertException>(async () => await driver.Title(),
                ex => Assert.That(ex.AlertText, Is.EqualTo("cheese")));
        }
