...
using Size = Zu.WebDriver.BasicTypes.WebSize;
using Point = Zu.WebDriver.BasicTypes.WebPoint;
...
namespace Zu.ZuChromeDriver.Tests
{
    ...
    public class PositionAndSizeTest : DriverTestFixture
    {
        ...
        [Test]
        public async Task ShouldHandleNonIntegerPositionAndSize()
        {
            await driver.GoToUrl(rectanglesPage);

            IWebElement r2 = await driver.FindElement(By.Id("r2"));
            string left = await r2.GetCssValue("left");
            Assert.That(Math.Round(decimal.Parse(left.Replace("px", ""), CultureInfo.InvariantCulture), 1), Is.EqualTo(10.9m));
            string top = await r2.GetCssValue("top");
            Assert.That(Math.Round(decimal.Parse(top.Replace("px", ""), CultureInfo.InvariantCulture), 1), Is.EqualTo(10.1m));
            Assert.AreEqual(new Point(11, 10), await r2.Location());
            string width = await r2.GetCssValue("width");
            Assert.That(Math.Round(decimal.Parse(width.Replace("px", ""), CultureInfo.InvariantCulture), 1), Is.EqualTo(48.7m));
            string height = await r2.GetCssValue("height");
            Assert.That(Math.Round(decimal.Parse(height.Replace("px", ""), CultureInfo.InvariantCulture), 1), Is.EqualTo(49.3m));
            Assert.AreEqual(new Size(49, 49), await r2.Size());
        }
