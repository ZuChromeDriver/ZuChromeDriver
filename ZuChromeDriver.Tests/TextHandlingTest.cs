...
namespace Zu.ZuChromeDriver.Tests
{
    ...
    public class TextHandlingTest : DriverTestFixture
    {
        ...
        [Test]
        [Ignore("Hidden LTR Unicode marks in getText; Selenium IgnoreBrowser(All), issue 4473")]
        public async Task ShouldNotReturnLtrMarks()
        {
            await driver.GoToUrl(EnvironmentManager.Instance.UrlBuilder.WhereIs("utf8/unicode_ltr.html"));
            IWebElement element = await driver.FindElement(By.Id("EH")).FindElement(By.TagName("nobr"));
            string text = await element.Text();
            const int ltrMark = 8206;
            Assert.That((int)text[0], Is.Not.EqualTo(ltrMark), "RTL/LTR mark should not be present as first character");
            Assert.That(text, Is.EqualTo("Some notes"));
        }

        [Test]
        [Ignore("Not all multi-byte whitespace is trimmed in getText; Selenium IgnoreBrowser(All), issue 6072")]
        public async Task ShouldTrimTextWithMultiByteWhitespaces()
        {
            await driver.GoToUrl(simpleTestPage);
            string text = await driver.FindElement(By.Id("trimmedSpace")).Text();
