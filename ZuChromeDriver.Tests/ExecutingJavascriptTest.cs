...
namespace Zu.ZuChromeDriver.Tests
{
    ...
    {
        [TearDown]
        public async Task DefaultContentAfterJavascriptTest()
        {
            try
            {
                if (driver == null)
                ...
                await driver.SwitchTo().DefaultContent().ConfigureAwait(false);
            }
            catch
            {
            }
        ...
        [Test]
        public async Task ShouldAwaitResolvedPromiseFromExecuteScriptBody()
        {
            if (!(driver is IJavaScriptExecutor))
                return;

            await driver.GoToUrl(xhtmlTestPage);
            object result = await ExecuteScript("return await Promise.resolve(42);");
            Assert.That(Convert.ToInt64(result), Is.EqualTo(42L));
        }

        [Test]
        public async Task ShouldAwaitDelayedPromiseFromExecuteScriptBody()
        {
            if (!(driver is IJavaScriptExecutor))
                return;

            await driver.GoToUrl(xhtmlTestPage);
            object result = await ExecuteScript("return await new Promise(r => setTimeout(() => r(7), 20));");
            Assert.That(Convert.ToInt64(result), Is.EqualTo(7L));
        }
