...
//using Zu.ZuChromeDriver.Tests.Remote;
...
namespace Zu.ZuChromeDriver.Tests
{
    public abstract class DriverTestFixture
    {
        ...
        protected ZuWebDriver driver;

        public ZuWebDriver DriverInstance
        {
            get { return driver; }
            set { driver = value; }
        }
        ...
        /// <summary>
        /// When true, the driver is created with <see cref="Zu.Chrome.ChromeDriverConfig.EnableFrameTrackerOnConnect"/> (iframe contexts and JS dialogs).
        /// </summary>
        protected virtual bool RequiresFrameTracker => false;

        public bool UsesFrameTracker => RequiresFrameTracker;

        [OneTimeSetUp]
        public void SetUp()
        {
            driver = RequiresFrameTracker
                ? EnvironmentManager.Instance.CreateFreshDriver(enableFrameTracker: true)
                : EnvironmentManager.Instance.GetCurrentDriver();
        }
        ...
        [TearDown]
        public async Task ResetAfterTest()
        {
            if (RequiresFrameTracker && driver != null)
            {
                try
                {
            ...
            {
                driver = EnvironmentManager.Instance.CreateFreshDriver(RequiresFrameTracker);
            }
        }
         ...
         */
        protected void CreateFreshDriver(bool enableFrameTracker = false)
        {
            driver = EnvironmentManager.Instance.CreateFreshDriver(enableFrameTracker);
        }
