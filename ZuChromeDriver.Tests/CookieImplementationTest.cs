...
namespace Zu.ZuChromeDriver.Tests
{
    ...
    public class CookieImplementationTest : DriverTestFixture
    {
        ...
        {
            await GotoValidDomainAndClearCookies("simpleTest.html");
        ...
        public async Task CannotGetCookiesWithPathDifferingOnlyInCase()
        {
            ...
            Assert.That(await driver.Options().Cookies.GetCookieNamed(cookieName), Is.Null);
        }
        ...
        public async Task ShouldBeAbleToSetDomainToTheCurrentDomain()
        {
            ...
            Uri url = new(await driver.GetUrl());
        ...
        public async Task ShouldWalkThePathToDeleteACookie()
        {
            ...
            Cookie cookie3 = new("dog", "dalmation", "/" + basePath + "/child/grandchild/");
            ...
            Assert.That(await driver.Options().Cookies.GetCookieNamed("rodent"), Is.Null);
        ...
        public async Task ShouldIgnoreThePortNumberOfTheHostWhenSettingTheCookie()
        {
            ...
            Uri uri = new(await driver.GetUrl());
        ...
        [Test]
        [Ignore("Local HtmlForTests has no HTTPS listener on the secure port (cf. upstream Ignore).")]
        ...
        public async Task ShouldRetainCookieSecure()
        {
            ...
            ReturnedCookie addedCookie = new("fish", "cod", string.Empty, "/common/animals", null, true, false);
        ...
        {
            string path = "/" + EnvironmentManager.Instance.UrlBuilder.Path;
            ...
            url.Append("&path=").Append(Uri.EscapeDataString(path));
        ...
        public async Task SettingACookieThatExpiredInThePast()
        {
            ...
            string p = "/" + EnvironmentManager.Instance.UrlBuilder.Path + "/animals";
            Cookie cookie = new("expired", "yes", p, expires);
            IOptions options = driver.Options();
        ...
        [TestCase("")]
        [TestCase("   ")]
        public async Task ShouldThrowWhenGetInvalidCookieByName(string cookieName)
        {
            try {
                await driver.Options().Cookies.GetCookieNamed(cookieName);
        ...
        [TestCase("")]
        [TestCase("   ")]
        public async Task ShouldThrowWhenDeleteInvalidCookieByName(string cookieName)
        {
            try {
                await driver.Options().Cookies.DeleteCookieNamed(cookieName);
                Assert.Fail("Expected ArgumentException");
            }
            catch (ArgumentException) {
            }
        ...
        [Test]
        public async Task ShouldThrowExceptionWhenAddingCookieToCookieAverseDocument()
        {
            if (!CheckIsOnValidHostNameForCookieTests()) {
                return;
            }
            ...
            await AssertEx.ThrowsAsync<WebBrowserException>(async () => await options.Cookies.AddCookie(cookie),
                ex => Assert.AreEqual("InvalidCookieDomainException", ex.Error));
        }
        ...
        public async Task ShouldNotBeAbleToSetDomainToSomethingThatIsUnrelatedToTheCurrentDomain()
        {
            ...
            Assert.That(await options.Cookies.GetCookieNamed("fish"), Is.Null);
        }
        ...
        public async Task ShouldAddCookieToCurrentDomainAndPath()
        {
            ...
            IOptions options = driver.Options();
            Cookie cookie = new("Homer", "Simpson", _hostname, "/" + EnvironmentManager.Instance.UrlBuilder.Path, null);
            await options.Cookies.AddCookie(cookie);
            ReadOnlyCollection<Cookie> cookies = await options.Cookies.AllCookies();
        ...
        private async Task<string> GetDocumentCookieOrNull()
        {
            ...
            try {
                ...
                // e.g. document.cookie not readable on this document (opaque origin / about:blank)
                return null;
            }
        }
        ...
        {
            Assert.That(await driver.Options().Cookies.GetCookieNamed(key), Is.Null, "Cookie was present with name " + key);
            string documentCookie = await GetDocumentCookieOrNull();
            if (documentCookie != null) {
        ...
        {
            Assert.That(await driver.Options().Cookies.GetCookieNamed(key), Is.Not.Null, "Cookie was present with name " + key);
            string documentCookie = await GetDocumentCookieOrNull();
            if (documentCookie != null) {
        ...
        {
            Assert.AreEqual(value, (await driver.Options().Cookies.GetCookieNamed(key)).Value, "Cookie had wrong value");
            string documentCookie = await GetDocumentCookieOrNull();
            if (documentCookie != null) {
