...
function callFunction(shadowHostIds, func, args, w3c, opt_unwrappedReturn) {
  ...
  try {
    var tmp = func.apply(null, unwrap(args, cache));
    return PromiseCtor.resolve(tmp).then(function(result) {
      return {
        status: 0,
        value: wrap(result)
      };
    }).catch(function(error) {
      var statusCode = error && error.code ? error.code : StatusCode.UNKNOWN_ERROR;
