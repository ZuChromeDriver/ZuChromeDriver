...
namespace Zu.WebDriver
{
    public class RemoteTargetLocator : IWebDriverTargetLocator
    {
        ...
        public RemoteTargetLocator(ZuWebDriver driver)
        {
            this.driver = driver;
        }
        ...
        public Task<IAlert> Alert(CancellationToken cancellationToken = new CancellationToken())
        {
            if (driver?.browserClient == null)
                throw new WebDriverException("no browserClient");
            if (driver.browserClient is IChromeDriver acd
                && (acd.FrameTracker == null || !acd.FrameTracker.TryGetBlockingJavaScriptDialog(out _)))
            {
                throw new NoAlertPresentException();
            }

            return Task.FromResult(driver.browserClient.Alert);
        }

        public async Task<IWebDriver> DefaultContent(CancellationToken cancellationToken = new CancellationToken())
        {
            await driver.browserClient.TargetLocator.SwitchToDefaultContent(cancellationToken).ConfigureAwait(false);
            return driver;
        }
