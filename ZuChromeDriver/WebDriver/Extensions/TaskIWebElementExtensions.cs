...
namespace Zu.WebDriver
{
    public static class TaskIWebElementExtensions
    {
        ...
        public static async Task<string> GetProperty(this Task<IWebElement> elementTask, string propertyName, CancellationToken cancellationToken = new CancellationToken())
        {
            var el = await elementTask.ConfigureAwait(false);
            return await el.GetProperty(propertyName, cancellationToken).ConfigureAwait(false);
        }
