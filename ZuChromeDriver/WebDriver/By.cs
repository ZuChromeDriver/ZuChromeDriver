...
namespace Zu.WebDriver
{
    ...
    public class By
    {
        ...
        public static By Name(string nameToFind)
        {
            ...
            var by = new By();
            var cssNameSelector = $"*[name =\"{EscapeCssSelector(nameToFind)}\"]";
            var nameXPath = nameToFind.Contains('\'', StringComparison.Ordinal)
                ? "//*[@name=\"" + nameToFind.Replace("\\", "\\\\", StringComparison.Ordinal).Replace("\"", "\\\"", StringComparison.Ordinal) + "\"]"
                : "//*[@name='" + nameToFind + "']";
            by._findElementMethod = async (context, cancellationToken) =>
            {
                if (context is IFindsByName findsByName)
                    return await findsByName.FindElementByName(nameToFind, cancellationToken).ConfigureAwait(false);
                if (context is IFindsByCssSelector findsByCss)
                    return await findsByCss.FindElementByCssSelector(cssNameSelector, cancellationToken).ConfigureAwait(false);
                if (context is IFindsByXPath findsByXpath)
                    return await findsByXpath.FindElementByXPath(nameXPath, cancellationToken).ConfigureAwait(false);
                throw new InvalidOperationException("The search context does not support locating by name.");
            };
            by._findElementsMethod = async (context, cancellationToken) =>
            {
                if (context is IFindsByName findsByName)
                    return await findsByName.FindElementsByName(nameToFind, cancellationToken).ConfigureAwait(false);
                if (context is IFindsByCssSelector findsByCss)
                    return await findsByCss.FindElementsByCssSelector(cssNameSelector, cancellationToken).ConfigureAwait(false);
                if (context is IFindsByXPath findsByXpath)
                    return await findsByXpath.FindElementsByXPath(nameXPath, cancellationToken).ConfigureAwait(false);
                throw new InvalidOperationException("The search context does not support locating by name.");
            };
