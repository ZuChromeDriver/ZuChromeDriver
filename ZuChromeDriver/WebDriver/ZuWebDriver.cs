...
{
    public class ZuWebDriver : IWebDriver
    {
        /// <summary>
        ...
        public ZuWebDriver(IAsyncWebBrowserClient client)
        {
            browserClient = client;
            Mouse = browserClient.Mouse;
            Keyboard = browserClient.Keyboard;
            storage = client.Options;
            appPath = Path.GetDirectoryName(typeof (ZuWebDriver).Assembly.Location);
        }
...
#region FindElement
        ...
        {
            // Match Selenium InvalidCharsRegex: space must be escaped in CSS id/hash selectors
        ...
        public IWebElement GetElementFromResponse(JsonNode response)
        {
            if (response == null)
                throw new WebBrowserException("Element not found", "no such element");
            ...
            if (json is JsonValue jv)
            {
                if (jv.GetValueKind() == JsonValueKind.Null)
                    throw new WebBrowserException("Element not found", "no such element");
                return CreateElement(jv.GetValue<string>())
                    ?? throw new WebBrowserException("Element not found", "no such element");
            }
        ...
        public ReadOnlyCollection<IWebElement> GetElementsFromResponse(JsonNode response)
        {
            var toReturn = new List<IWebElement>();
            JsonArray arr = null;
            if (response is JsonArray directArr)
                arr = directArr;
            else if (response is JsonObject)
                arr = WebDriverJsonHelper.UnwrapWebDriverValueContainer(response) as JsonArray;
            if (arr == null)
        ...
        /// </code>
        /// </example>
        public IWebDriverTargetLocator SwitchTo()
        {
        ...
        public async Task<string> GetUrl(CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            {
                throw new WebBrowserException(ex.Message, "InvalidOperationException");
            }
        }
        ...
        public async Task<string> Title(CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            {
                DriverCoreExceptionMapper.RethrowMapped(ex);
                throw;
            }
        }
        ...
        /// <summary>
        ///     Creates a <see cref = "WebElement"/> with the specified ID.
        /// </summary>
        /// <param name = "elementId">The ID of this element.</param>
        /// <returns>A <see cref = "WebElement"/> with the specified ID.</returns>
        public virtual WebElement CreateElement(string elementId)
        {
            if (string.IsNullOrWhiteSpace(elementId))
                return null;
            var toReturn = new WebElement(this, elementId);
            toReturn.SimpleCommandsTimeoutMs = SimpleCommandsTimeoutMs;
            return toReturn;
        }
        ...
        private static object ConvertObjectToJavaScriptObject(object arg)
        {
            ...
            else if (argAsElement != null)
            {
                ...
                {
                    { "ELEMENT", argAsElement.InternalElementId },
                    { "element-6066-11e4-a52e-4f735466cecf", argAsElement.InternalElementId }
                };
        ...
        private static string CoerceScriptResultToElementId(object value)
        {
            if (value == null)
                return null;
        ...
        private object ParseJavaScriptReturnValue(object responseValue)
        {
            ...
            if (resultAsDictionary != null)
            {
                ...
                {
                    string id = CoerceScriptResultToElementId(resultAsDictionary["element-6066-11e4-a52e-4f735466cecf"]);
                    var element = this.CreateElement(id);
                    returnValue = element;
                }
                else if (resultAsDictionary.ContainsKey("ELEMENT"))
                {
                    string id = CoerceScriptResultToElementId(resultAsDictionary["ELEMENT"]);
                    var element = this.CreateElement(id);
                    returnValue = element;
                }
                else
...
#region WaitForElement
        ...
        public Task PerformActions(IList<ActionSequence> actionSequenceList, CancellationToken cancellationToken = default)
        {
            if (browserClient == null)
                throw new WebDriverException("no browserClient");
