...
namespace Zu.WebDriver
{
    ...
    public interface IWebDriver : IDisposable, ISearchContext, IJavaScriptExecutor, IFindsById, IFindsByClassName, IFindsByLinkText, IFindsByName, IFindsByTagName, IFindsByXPath, IFindsByPartialLinkText, IFindsByCssSelector, ITakesScreenshot, IHasInputDevices, IHasWebStorage, IHasLocationContext, IHasApplicationCache, IActionExecutor
    {
        ...
        /// </summary>
        /// <returns>
        ///     An <see cref="IWebDriverTargetLocator" /> object which can be used to select
        ///     a frame or window.
        /// </returns>
        IWebDriverTargetLocator SwitchTo();

        Task ClearElement(string elementId, CancellationToken cancellationToken = default);
        Task ClickElement(string elementId, CancellationToken cancellationToken = default);
    }
}