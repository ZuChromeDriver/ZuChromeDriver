...
using CdpLogEntry = Zu.ChromeDevTools.Log.LogEntry;
...
namespace Zu.WebDriver.BasicTypes
{
    ...
    public class LogEntry
    {
        ...
        private LogEntry(DateTime timestamp, LogLevel level, string message)
        {
            Timestamp = timestamp;
            Level = level;
        ...
        /// <summary>
        /// Maps a CDP <see cref="Zu.ChromeDevTools.Log.LogEntry"/> to the WebDriver-style log entry used by tests.
        /// </summary>
        public static LogEntry FromChromeDevTools(CdpLogEntry entry)
        {
            if (entry == null)
            ...
            DateTime timestamp = ts > 1e12
                ? epoch.AddMilliseconds(ts)
                : epoch.AddSeconds(ts);

            return new LogEntry(timestamp, MapCdpLevel(entry.Level), entry.Text ?? string.Empty);
        }

        static LogLevel MapCdpLevel(string level)
        {
